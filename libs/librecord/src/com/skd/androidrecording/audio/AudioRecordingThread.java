...
public class AudioRecordingThread extends Thread {
	...
	public AudioRecordingThread(String fileWavName,
			AudioRecordingHandler handler) {
	...
	public void run() {
		...
		while (isRecording) {
			...
			if ((read == AudioRecord.ERROR_INVALID_OPERATION)
					|| (read == AudioRecord.ERROR_BAD_VALUE) || (read <= 0)) {
				continue;
			}
	...
	private void proceed() {
		...
		/*
		 * See
		 * http://developer.android.com/reference/android/media/audiofx/Visualizer
		 * .html#getFft(byte[]) for format explanation
		 */
	...
	private void convertRawToWav() {
		...
		try {
			PcmAudioHelper.convertRawToWav(WavAudioFormat.mono16Bit(32),
					file_raw, file_wav);
			file_raw.delete();
			if (handler != null) {
